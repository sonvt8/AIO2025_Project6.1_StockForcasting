...
def fetch_fpt_data(days: int = 120) -> pd.DataFrame:
    ...
    Strategy:
    1. Load existing dataset to get last date
    ...
    Args:
        days: Minimum number of days needed (for backward compatibility,
              but actual fetch will be based on dataset last date)

    Returns:
        DataFrame with columns: time, open, high, low, close, volume, symbol
        Format matches FPT_train.csv structure
        Contains all data from dataset + newly fetched data
    """
    ...
    # If we have existing data and last_date is up to date, return existing data
    if existing_df is not None and last_date is not None:
        ...
        if days_since_last <= 2:
            # Data is up to date, return existing data
            return existing_df
